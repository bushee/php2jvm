...
class InstanceFunctionDefinitionSpec extends Specification {
    def method = InstanceFunctionDefinitionSpec.getMethods().find { it.name == 'method' }
    def methodWithDefaults = InstanceFunctionDefinitionSpec.getMethods().find { it.name == 'methodWithDefaults' }
    def staticMethod = InstanceFunctionDefinitionSpec.getMethods().find { it.name == 'staticMethod' }

    def "should throw exception if trying to instantiate InstanceFunctionDefinition with a static method"() {
        when:
        new InstanceFunctionDefinition(this, staticMethod)
    ...
    def "calling function with parameters should delegate to actual passed method"() {
        given:
        def functionDefinition = new InstanceFunctionDefinition(this, method)
    ...
    def "calling function with missing parameters should result in using default values for them (passing #argsCount out of 9 arguments)"() {
        given:
        def functionDefinition = new InstanceFunctionDefinition(this, methodWithDefaults)
